...
describe("Central de Atendimento ao Cliente TAT", function () {
  ...
  it("Preenche campos obrigatórios e envia formulário", () => {
    ...
    cy.get("#open-text-area").type(
      "Prezados, gostaria de expressar minha insatisfação com o serviço prestado. Comprei um produto há duas semanas e até agora não o recebi. Entrei em contato com o atendimento ao cliente várias vezes, mas não obtive nenhuma resposta satisfatória. O número do meu pedido é 123456. Aguardo uma solução rápida para o problema.",
      { delay: 0 }
    );
  ...
  it("Preenche e limpa os campos nome, sobrenome, email e telefone", () => {
    ...
    cy.get("#email")
      .type("bruna.campos@gmail.com")
      .should("have.value", "bruna.campos@gmail.com");
    cy.get("#open-text-area")
      .type(
        "Prezados, gostaria de expressar minha insatisfação com o serviço prestado. Comprei um produto há duas semanas e até agora não o recebi.",
        { delay: 0 }
      )
      .should(
        "have.value",
  ...
  it("Seleciona um arquivo da pasta fixtures", () => {
    cy.get('input[type="file"]')
      .selectFile("cypress/fixtures/Arquivo_teste.txt")
      .then((input) => {
  ...
  it("Seleciona um arquivo simulando um drag-and-drop", () => {
    cy.get('input[type="file"]')
      .selectFile("cypress/fixtures/Arquivo_teste.txt", { action: "drag-drop" })
      .then((input) => {
        expect(input[0].files[0].name).to.equal("Arquivo_teste.txt");
      });
  });
  ...
  Cypress._.times(2, () => {
    it.only("Congela e avança o tempo para a verificação de mensagem", () => {
      cy.clock();
      cy.get("#firstName").type("Bruna");
      cy.get("#lastName").type("Campos");
      cy.get("#email").type("bruna.campos@gmail.com");
